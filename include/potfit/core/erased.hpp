...
protected:
    ...
    explicit ErasedValue(std::unique_ptr<Concept> p) noexcept : self_(std::move(p)) {}
...
template <typename Concept>
class ErasedMoveOnly {
protected:
    std::unique_ptr<Concept> self_;
    explicit ErasedMoveOnly(std::unique_ptr<Concept> p) noexcept : self_(std::move(p)) {}
