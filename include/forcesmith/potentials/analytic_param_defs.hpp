...
// function here + a row in FORCESMITH_ANALYTIC_FUNCTIONS below is all it takes to
...
#define FORCESMITH_APD_morse_sc(X) FORCESMITH_APD_morse(X) FORCESMITH_APD_SC_H(X)
#define FORCESMITH_APD_exp_decay_sc(X) FORCESMITH_APD_exp_decay(X) FORCESMITH_APD_SC_H(X)
...
#define FORCESMITH_APD_AS_NAME(tok, v, lo, hi) std::string(BOOST_PP_STRINGIZE(tok)),
