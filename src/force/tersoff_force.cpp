...
double fc_val(double r, double R, double S) noexcept {
    if (r <= R) return 1.0;
    if (r >= S) return 0.0;
...
double dfc_val(double r, double R, double S) noexcept {
...
double g_val(double c, const TersoffParams& p) noexcept {
...
// dg/d(cos θ)
double dg_val(double c, const TersoffParams& p) noexcept {
    const double c2  = p.c * p.c;
    const double d2  = p.d * p.d;
    const double hc  = p.h - c;
...
double bond_order(double zeta, const TersoffParams& p) noexcept {
...
double dbond_dzeta(double zeta, const TersoffParams& p) noexcept {
...
void TersoffForceCalculator::eval_forces(Configuration& cfg) const {
    ...
    for (int ii = 0; ii < natoms; ++ii) {
        ...
        for (int jj = 0; jj < nn; ++jj) {
            ...
            if (fc_ij == 0.0 && dfc_ij == 0.0) continue;
            ...
            for (int kk = 0; kk < nn; ++kk) {
                ...
                if (fc_ik == 0.0 && dfc_ik == 0.0) continue;
                ...
                const Vec3 dz_dri = -dfc_ik * inv_r2 * gv * d2
                                  + fc_ik * dgv * (Ac * d1 + Bc * d2);

                // ∂ζ/∂r_j = fc_ik × dgv × (d2/(r1 r2) − c d1/r1²)
                const Vec3 dz_drj = fc_ik * dgv *
                                    (inv_r1 * inv_r2 * d2 - cos_theta * inv_r1 * inv_r1 * d1);

                // ∂ζ/∂r_k = g × dfc_ik × (d2/r2) + fc_ik × dgv × (d1/(r1 r2) − c d2/r2²)
                const Vec3 dz_drk = dfc_ik * inv_r2 * gv * d2
                                  + fc_ik * dgv *
                ...
                cfg.calc_stress += 0.5 * (d1 * (-Fj_3b).transpose()
                                        + d2 * (-Fk_3b).transpose());
            }
        }
    }
