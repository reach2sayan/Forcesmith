...
// bracketing + Brent). Derivative-free; each sweep line-minimises along every
...
static_assert(SolverImpl<LineSearchSolver>);

// Powell's direction-set minimiser over φ(x) = ½‖F(x)‖², built around linmin.
// Held as a small stateful helper so LineSearchSolver::minimize reads as a
// sequence of named stages: objective → directional line search → full sweep →
...
struct PowellDirectionSet {
  ...
  std::optional<Eigen::VectorXd> conjugate_direction(const Eigen::VectorXd &p0,
                                                     const Eigen::VectorXd &x,
                                                     double fp, double fret,
                                                     double del) const;
};
