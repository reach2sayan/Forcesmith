    ...
    for (auto& ai : cfg.atoms) {
        ...
        for (int jj = 0; jj < nn; ++jj) {
            ...
            for (int kk = jj + 1; kk < nn; ++kk) {
                ...
                const double g   = angular(nb_j.neighbor->type, nb_k.neighbor->type).eval(c);
                const double dg  = angular(nb_j.neighbor->type, nb_k.neighbor->type).deriv(c);
                ...
                const Vec3 fj = -(df1 * inv_r1 * f2 * g) * d1
                              - (f1 * f2 * dg) * (inv_r1r2 * d2 - c * inv_r1 * inv_r1 * d1);

                const Vec3 fk = -(f1 * df2 * inv_r2 * g) * d2
