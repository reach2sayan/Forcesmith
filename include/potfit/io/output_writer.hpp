...
//   ADP:     {model:"adp",     ntypes, pair, density, embedding, dipole, quadrupole}
//   Angular: {model:"angular", ntypes, pair, radial, angular}
void write_native_adp(const std::filesystem::path &path,
                      const ADPForceCalculator &adp, int nknots = kDefaultKnots);
...
//   Tersoff: {model:"tersoff", ntypes, potentials:[{A,B,lambda,mu,beta,n,c,d,h,R,S,omega}]}
//   Stiweb:  {model:"stiweb",  ntypes, potentials:[{A,B,p,q,delta,a1,gamma,a2}], lambda:[…]}
